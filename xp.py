...
class ExperienceCog(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        async with aiosqlite.connect("database.sqlite") as db:
            ...
            if f"{message.author.id}.{message.guild.id}" in self.last_message_in_guild:
                ...
                async with db.execute("SELECT time_between_xp FROM server_config WHERE guild=?",
            ...
            async with db.execute("SELECT userorchannel FROM guild_xp_exclusions WHERE guild=?",
                                  (message.guild.id,)) as cur:
                cur: aiosqlite.Cursor
            ...
            await db.execute("""INSERT INTO experience(user, guild, experience) VALUES (?,?,1)
        ...
        async with aiosqlite.connect("database.sqlite") as db:
            async with db.execute("SELECT 1 FROM guild_xp_exclusions WHERE guild=? AND userorchannel=?",
                ...
                if await cur.fetchone() is not None:
                    ...
                    exists = False
                    await db.execute("INSERT INTO guild_xp_exclusions(guild, userorchannel, mod_set) VALUES (?,?,true)",
                                     (ctx.guild.id, ctx.author.id))
                await db.commit()
    ...
    async def togglemyxp(self, ctx: commands.Context):
        ...
        """
        async with aiosqlite.connect("database.sqlite") as db:
            async with db.execute("SELECT mod_set FROM guild_xp_exclusions WHERE guild=? AND userorchannel=?",
                                  (ctx.guild.id, ctx.author.id)) as cur:
